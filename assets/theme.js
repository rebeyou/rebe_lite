...
$( document ).ready(function() {
    ...
    $('.btn-play').click(function(){
        ...
        $('#video-Play').html('<video src="'+link+'" id="Mp4264a6801-17c3-48cf-8cb4-fc61d8c5caed" loop="" muted="" playsinline="" autoplay=""></video>');
        $(".video-banner").addClass('video-pause');
    });
    ...
    $('.close-btn').click(function(){
        ...
        $('.mainVideo-1').fadeOut();
        $('#video-Play').html('');
        $(".video-banner").removeClass('video-pause');
    });
...
$(document).ready(function () {
 ...
 //if the link has the class
  	$('.newsletterCustom > .container').after($('<ul class="list-unstyled shape-group-9"><li class="shape shape-1"><img src="https://cdn.shopify.com/s/files/1/0522/8678/3654/files/bubble-12.png" alt="Comments"></li><li class="shape shape-2"><img src="https://cdn.shopify.com/s/files/1/0522/8678/3654/files/bubble-16.png" alt="Comments"></li><li class="shape shape-3"><img src="https://cdn.shopify.com/s/files/1/0522/8678/3654/files/bubble-13.png" alt="Comments"></li><li class="shape shape-4"><img src="https://cdn.shopify.com/s/files/1/0522/8678/3654/files/bubble-14.png" alt="Comments"></li><li class="shape shape-5"><img src="https://cdn.shopify.com/s/files/1/0522/8678/3654/files/bubble-16.png" alt="Comments"></li><li class="shape shape-6"><img src="https://cdn.shopify.com/s/files/1/0522/8678/3654/files/bubble-15.png" alt="Comments"></li><li class="shape shape-7"><img src="https://cdn.shopify.com/s/files/1/0522/8678/3654/files/bubble-16.png" alt="Comments"></li></ul>'));
...
$(document).ready(function () {
    ...
    var $set = $('.sectionThreeBlocks');
  
    for(var i=0, len = $set.length; i < len; i+=3){
      	$set.removeClass('combineSection');
        $set.slice(i, i+3).wrapAll('<div class="mainSection-box sectionThreeInner"><div class="container"></div></div>');
    }   
...
$(document).ready(function () {
    ...
    var elems = $(".combineSection");
    var wrapper = $('<div class="mainSection-box"><div class="container"></div></div>');
      ...
      if ($productThumbs.length > 1) {
        ...
        if (this.settings.sectionId === 'product-template-3' || this.settings.sectionId === 'product-template-4'){
          ...
          if ($(window).width() > 767) {
            ...
            var refreshScrollOffsets = function() {
              topMenuHeight = $('.site-header').outerHeight() + 20;
              scrollItemOffsets = scrollItems.map(function() {
                return $(this).offset().top;
              }).get();
            };

            var scrollItemOffsets = [];
            refreshScrollOffsets();

            var updateActiveThumbnail = $.debounce(40, function() {
                var fromTop = (window.pageYOffset || $(window).scrollTop()) + topMenuHeight;
                var activeIndex = -1;

                for (var i = 0; i < scrollItemOffsets.length; i++) {
                  if (scrollItemOffsets[i] < fromTop) {
                    activeIndex = i;
                  } else {
                    break;
                  }
                ...
                var activeItem = activeIndex >= 0 ? scrollItems.eq(activeIndex) : $();
                var id = activeItem.length ? activeItem[0].id : '';
            ...
            $(window).on('resize', $.debounce(120, function() {
              refreshScrollOffsets();
              updateActiveThumbnail();
            }));

            $(window).on('scroll', updateActiveThumbnail);
          }
          else{
            menuItems.click(function (e){
              var findid =$(this).attr('href');
    ...
    this.settings = {
      ...
      customPaging: function(slider, i) {
        return $('<button>', {
          ...
          $('<span>', { 'aria-hidden': 'true', text: i + 1 })
        );
      },
      init: function() {
    ...
    if (this.$dots) {
      ...
      .each(function() {
        ...
        $dot.on('click keyup', function(evt) {
            ...
            var slideNumber = $(evt.target).closest('li').index();
...
theme.Instagrams = (function() {
  ...
  theme.bindSlickA11y = theme.bindSlickA11y || function($container, obj, beforeBind) {
    var $list = obj && obj.$list;

    if (!$list || typeof $list.removeAttr !== 'function') {
      $list = obj && obj.$slider && obj.$slider.find
        ? obj.$slider.find('.slick-list')
        : $container && $container.find
          ? $container.find('.slick-list')
          : $();
    }
  ...
  Instagrams.prototype = _.assignIn({}, Instagrams.prototype, {
    _a11y: function(event, obj) {
      theme.bindSlickA11y(this.$container, obj);
    }
  });
...
theme.slickCarousel = (function (){
  ...
  Carousels.prototype = _.assignIn({}, Carousels.prototype, {
    _a11y: function(event, obj) {
      theme.bindSlickA11y(this.$container, obj, function($container) {
        // ARIA fix: label slick-track listbox for screen readers
        var $track = $container.find('.slick-track');
        $track.attr('role', 'region');

        if ($container.hasClass('brand-slider')) {
          $track.attr('aria-label', 'Partner logos');
        } else if ($container.hasClass('carousel-slider')) {
          $track.attr('aria-label', 'Customer reviews');
        } else {
          $track.attr('aria-label', 'Content carousel');
        }
      });
    },
...
// Fix: aria-hidden + focusable descendants in #jsLogin modal
document.addEventListener("DOMContentLoaded", function(){
  var loginModal = document.getElementById('jsLogin');
  if (loginModal) {
...
// Fix: aria-hidden + focusable descendants in #jsQuickview modal
document.addEventListener("DOMContentLoaded", function(){
  var qvModal = document.getElementById('jsQuickview');
  if (qvModal) {
    qvModal.setAttribute('inert', '');
    $(qvModal).on('show.bs.modal', function () {
      this.removeAttribute('inert');
    }).on('hidden.bs.modal', function () {
      this.setAttribute('inert', '');
    });
...
// Fix: focusable elements in aria-hidden Slick slides (slideshow)
document.addEventListener("DOMContentLoaded", function(){
  function fixSlickFocus(slickContainer) {
    $(slickContainer).find('.slick-slide[aria-hidden="true"]').find('a, button, input, [tabindex]').attr('tabindex', '-1');
    $(slickContainer).find('.slick-slide:not([aria-hidden="true"])').find('a, button, input, [tabindex]').attr('tabindex', '0');
  }

  $(document).on('init afterChange', '.slideshow', function() {
    fixSlickFocus(this);
  });
});
...
theme.cookie = (function(){
  var $closeButton = $('.js-btn-ok'),
      isOk = localStorage.getItem('localCookie') || '';
  
  if (isOk === '') {
    window.addEventListener('load', function() {
      setTimeout(function() {
        $('.js-cookie-policy').fadeIn('slow');
      }, 1000);
    });
  }
...
theme.isotope = (function(){
  ...
  function initIsotope(){
    if (enableIsotope){
      ...
      var relayoutGridOnScroll = $.debounce(120, function() {
        $grid.isotope('layout');
      });
      $(window).on('scroll', relayoutGridOnScroll);
      $(window).on('load', function() {
        $grid.isotope('layout');
...
theme.Masonry = (function(){
  function Masonry(container) {
    ...
    var relayoutMasonryOnScroll = $.debounce(120, function() {
      $masonry.isotope('layout').css('opacity','1');
    });
    $(window).on('scroll', relayoutMasonryOnScroll);
  }
...
theme.stickyHeader = (function(){
  ...
  if ($(stickHeaderClass).length !== 0){
    var toggleStickyHeader = $.debounce(50, function() {
...
theme.ZoomImage = (function(){
  var onResize = $.debounce(150, function() {
...
theme.stickyCart = (function(){
  ...
  if($anchor.length){
    function updateStickyCartThreshold() {
      var hT = $anchor.offset().top,
          hH = $anchor.outerHeight(),
          wH = window.innerHeight || $(window).height();
      stickyCartThreshold = hT + hH - wH;
    }

    var toggleStickyCart = $.debounce(50, function() {
      var wS = window.pageYOffset || $(window).scrollTop();
      if (wS > stickyCartThreshold){
    ...
    updateStickyCartThreshold();
    $(window).on('resize', $.debounce(150, updateStickyCartThreshold));
...
theme.backToTop = (function(){
  ...
  var toggleBackToTop = $.debounce(60, function() {
    if ((window.pageYOffset || $(window).scrollTop()) > 300) {
...
// Fix: Remove invalid aria-selected from Slick slides (role="group" incompatibility)
(function fixSlickAriaSelected() {
  ...
  // Fallback for already-initialized sliders
  document.addEventListener('DOMContentLoaded', removeAriaSelected);

  // Hook into Slick events if jQuery is present
  if (window.jQuery) {
    jQuery(document).on('init reInit afterChange', '[data-slick], .slick-initialized', function() {
      removeAriaSelected();
    });
  }

  // MutationObserver as a belt-and-suspenders fallback
  // (catches dynamically injected sliders)
  var observer = new MutationObserver(function(mutations) {
    mutations.forEach(function(mutation) {
      mutation.addedNodes.forEach(function(node) {
        if (node.nodeType === 1 && (
          node.classList.contains('slick-slide') ||
          node.querySelector && node.querySelector('.slick-slide')
        )) {
          removeAriaSelected();
        }
      });
