...
theme.slickCarousel = (function (){
 ...
 if (!$list || typeof $list.removeAttr !== 'function') {
      ...
      var $track = this.$container.find('.slick-track');
          $track.attr('role', 'region');

      if (this.$container.hasClass('brand-slider')) {
        $track.attr('aria-label', 'Partner logos');
      } else if (this.$container.hasClass('carousel-slider')) {
...
// Fix: Slick slides aria-hidden focusable descendants
$(document).on('init reInit afterChange', '.slideshow', function(event, slick) {
  $(this).find('.slick-slide[aria-hidden="true"]')
    .find('a, button, input, select, textarea, [tabindex]')
    .attr('tabindex', '-1');
  $(this).find('.slick-slide:not([aria-hidden="true"])')
